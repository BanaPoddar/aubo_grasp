...
@app.route('/api/pose_camera', methods=['POST'])
def api():
    global img_color
    global img_depth
    encoded_color = base64.b64encode(img_color).decode('utf-8')
    encoded_depth = base64.b64encode(img_depth).decode('utf-8')
    encoded_content = {"img_color": encoded_color, "img_depth": encoded_depth}
    #打印日志
    app.logger.info("Pose camera API called.")
    app.logger.info(encoded_content)
    return jsonify({"code": 200, "msg": "success", "data": encoded_content})

def run_image_sender():
    try:
...
if __name__ == "__main__":
        ...
        image_sender_thread = threading.Thread(target=run_image_sender)
