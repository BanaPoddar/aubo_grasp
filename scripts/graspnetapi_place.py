...
from aubo_grasp.msg import placingMessage
...
id=0
type="color"
place_flag = False#放置标志
can_capture_image = False#获取放置位置图像信息
place_color = "none"#放置颜色
place_id = 0#放置id
...
class MoveItAubo:
    ...
    def move_to_place(self):
        #定义关节角度  
        joint_group_positions = [0, 0, 0, 0, 0, 0]
        joint_group_positions[0] = 40 * np.pi / 180.0
        joint_group_positions[1] = 0
        joint_group_positions[2] = 60 * np.pi / 180.0
        joint_group_positions[3] = -30 * np.pi / 180.0
        joint_group_positions[4] = 90 * np.pi / 180.0
        joint_group_positions[5] = -50 * np.pi / 180.0
        self.arm.set_joint_value_target(joint_group_positions)
        self.arm.go()
        rospy.sleep(1)

    def move_to_final_place(self):
        #定义关节角度  
        joint_group_positions = [0, 0, 0, 0, 0, 0]
        joint_group_positions[0] = 40 * np.pi / 180.0
        joint_group_positions[1] = -12 * np.pi / 180.0
        joint_group_positions[2] = 80 * np.pi / 180.0
        joint_group_positions[3] = 2 * np.pi / 180.0
        joint_group_positions[4] = 90 * np.pi / 180.0
        joint_group_positions[5] = -50 * np.pi / 180.0
        self.arm.set_joint_value_target(joint_group_positions)
        self.arm.go()
        ...
        while True:
            ...
            if time.time()-start_time >= camera_setup_time:
                ...
                if place_flag:
                    ...
                    # self.moveit_aubo.move_to_final_place()
                    # self.moveit_aubo.gripper_control(0)
                    #等待机械臂移动到指定位置
                    self.moveit_aubo.move_to_place()
                    place_flag = False
                ...
                if can_capture_image:
                    ...
                    for result in results[0].boxes:
                        ...
                        #if cls == :
                        x1 = np.int64(result.xyxy[0][0].item())
                        y1 = np.int64(result.xyxy[0][1].item())
                        x2 = np.int64(result.xyxy[0][2].item())
                        y2 = np.int64(result.xyxy[0][3].item())
                        probability = result.conf.item()
                        depth_pixel = [int((x1 + x2) / 2), int((y1 + y2) / 2)]
                        dis, camera_coordinate = self.get_3d_camera_coordinate(depth_pixel, aligned_depth_frame, depth_intrin)
                        #显示一下图片
                        ...
                        if cv2.waitKey(1) & 0xFF == ord("q"):
                            break
                        rospy.loginfo("Object: %s, Probability: %f, Distance: %f, Camera Coordinate: %s" % (cls, probability, dis, camera_coordinate))
                        #移动到指定位置
...
if __name__ == "__main__":
    ...
    try:
        ...
        rospy.Subscriber('placingObject', placingMessage, place_callback)
        image_sender.run()
        rospy.spin()
